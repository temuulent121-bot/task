...
// --- Constants ---
const BUCKET_NAME = "found-items";
const TABLE_NAME = "found_items";
...
function validateField(input) {
    ...
    if (input.hasAttribute('required') && !input.value.trim()) {
        ...
        // Regex for basic email or phone number (digits with optional '+')
    ...
    } else if (input === inputs.dateFound) {
...
function setupImageUploadListeners() {
    ...
    imageFileInput.addEventListener('change', (e) => { const file = e.target.files[0]; if (file) handleFileSelected(file); });
...
async function handleFormSubmit(event) {
    ...
    try {
        ...
        const reportData = {
            ...
            location_found: inputs.locationFound.value.trim(),
            contact: inputs.contact.value.trim(),
            date_found: inputs.dateFound.value,
            image_url: imageUrl,
            user_id: user.id,
            status: 'Available' // Default status for a newly found item
        };

        const { error: insertError } = await supabase.from(TABLE_NAME).insert(reportData);
        if (insertError) throw new Error(`Database error: ${insertError.message}`);
        
        showFormMessage("Item reported successfully! Thank you for your help.", "success");
...
function setActiveNavLink() {
    const currentPageName = 'report-found.html';
