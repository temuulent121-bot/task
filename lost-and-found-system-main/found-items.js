...
async function loadItems() {
    ...
    try {
        const { data, error } = await supabase
            .from("found_items") // Querying the 'found_items' table
            .select("*")
            .eq('status', 'Active') // Filtering for 'Active' status
...
function renderItems() {
    ...
    filtered.sort((a, b) => {
        switch (sortBy) {
            case 'date_found_asc': return new Date(a.date_found) - new Date(b.date_found);
            case 'name_asc': return a.name.localeCompare(b.name);
            case 'name_desc': return b.name.localeCompare(a.name);
            default: return new Date(b.date_found) - new Date(a.date_found);
        }
    });
    ...
    if (filtered.length === 0) {
        ...
        itemsListContainer.innerHTML = `<p class="no-items-message"><i class="fas fa-search"></i> No active found items ${message}</p>`;
        return;
    }
        ...
        card.innerHTML = `
            ...
            <div class="item-info">
                ...
                <div class="item-meta">
                    ...
                    <span><i class="fas fa-calendar-alt"></i> ${new Date(item.date_found).toLocaleDateString()}</span>
                </div>
            </div>
        `;
...
function populateAndOpenEditModal(item) {
    ...
    editForm.querySelector('#editLocationFound').value = item.location_found;
    editForm.querySelector('#editDateFound').value = formatDate(item.date_found);
...
async function handleUpdateSubmit(event) {
    ...
    const updatedData = {
        ...
        location_found: editForm.querySelector('#editLocationFound').value,
        date_found: editForm.querySelector('#editDateFound').value,
    ...
    try {
        const { error } = await supabase
            .from('found_items') // Updating the 'found_items' table
            .update(updatedData)
            .eq('id', id);
...
const setActiveNavLink = () => {
    const currentPageName = 'found-items.html';
